...
module.exports = {
  ...
  askForMandrillConfig: () => {
    const questions = [
      {
        name: "key",
        type: "input",
        message: "Enter your Madrill Api Key:",
        validate: function (value) {
          if (value.length) {
            return true;
          } else {
            return "Please enter your Madrill api key";
          }
        },
      },
      {
        name: "sender_email",
        type: "input",
        message: "Enter your Madrill Sender Email:",
        validate: function (value) {
          if (value.length) {
            return true;
          } else {
      ...
      },
      {
        name: "sender_name",
        type: "input",
        message: "Enter your Madrill Sender Name:",
        validate: function (value) {
          if (value.length) {
            return true;
          } else {
  ...
  askForTrelloToken: async () => {
    const tokenUrl = `https://trello.com/1/authorize?expiration=30days&name=MyPersonalToken&scope=read&response_type=token&key=${config.get(
      "trello.key"
    )}`;
  ...
  askForGoogleAccessToken: async () => {
    const { oAuth2Client, authUrl } = await google.getAccessTokenUrl(
      config.get("google")
    );
