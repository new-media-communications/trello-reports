...
class TrelloBoard extends TrelloApi {
  ...
  async members() {
    ...
    this._members = (
      await this.client().get(`/boards/${this.board.id}/members`, {
        params: { fields: "all" },
      })
    ).data;
  ...
  async membersAndCards() {
    ...
    const cardsWithList = cards.map((c) => ({
      ...c,
  ...
  async _getAllCards() {
    ...
    let before = null;
    for (; true; ) {
      const cards = await this._getCards(this.board.id, {
        limit: 1000,
        before: before,
      });
