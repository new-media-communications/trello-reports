...
class GoogleSheetService {
  ...
  async init() {
    ...
    this._getSheet = util.promisify(
      sheets.spreadsheets.values.get.bind(sheets)
    );
    this._getSpreadsheet = util.promisify(sheets.spreadsheets.get.bind(sheets));
    this._batchUpdateSpreadsheet = util.promisify(
      sheets.spreadsheets.batchUpdate.bind(sheets)
    );
    this._copySheet = util.promisify(
      sheets.spreadsheets.sheets.copyTo.bind(sheets)
    );
    this._updateSheet = util.promisify(
      sheets.spreadsheets.values.batchUpdate.bind(sheets)
    );
  ...
  async autoResize() {
    const spreadsheet = await this.getSpreadsheet({ includeGridData: false });
  ...
  async copyOrGetSheet(sheetName) {
    ...
    try {
      const {
        data: { values: sheetValues },
      } = await this._getSheet({
        spreadsheetId,
        range: sheetName,
      });
      return sheetValues;
