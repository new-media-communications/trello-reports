...
async function updateSheetData(options, sheetsData) {
  ...
  const sheetsToDelete = sheets.filter(sheet => {
    const sheetData = sheetsData.find(s => s.sheetName === sheet.properties.title);
...
module.exports = { toSheetDataAll, toSheetDataByYearMonth, toSheetDataByUser, updateSheetData }